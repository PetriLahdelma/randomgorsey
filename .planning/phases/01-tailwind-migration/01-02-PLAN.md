...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/components/ui/button.tsx"
      provides: "shadcn Button component with Radix primitive"
      contains: "Slot"
  key_links:
    - from: "vite.config.ts"
      to: "@tailwindcss/vite"
...
<task type="auto">
  <name>Task 3: Create utilities, shadcn/ui configuration, and install first component</name>
  <files>
    src/lib/utils.ts
    components.json
    tsconfig.json
...
5. Install a shadcn/ui component to verify the setup works end-to-end:
   ```bash
   npx shadcn@latest add button
   ```
   This will:
   - Create src/components/ui/button.tsx with Radix Slot primitive
   - Verify all configuration is correct
   - Prove the shadcn/ui CLI works for future component additions

6. Verify the button component was created and contains Radix import:
   ```bash
   cat src/components/ui/button.tsx | grep -q "Slot" && echo "Radix Slot imported"
   ```
...
7. Commit changes:
   ...
   - Install shadcn Button component (verifies Radix integration)
  ...
  <verify>
   ...
   - src/components/ui/button.tsx exists and contains "Slot" import from @radix-ui/react-slot
   - Import `import { cn } from "@/lib/utils"` works in any component
   - Import `import { Button } from "@/components/ui/button"` works
  </verify>
  <done>cn() utility created, shadcn/ui CLI configured, Button component installed with Radix primitives</done>
</task>
...
3. shadcn/ui CLI works: `npx shadcn@latest add dialog --dry-run` shows what would be added
4. Radix primitives available: src/components/ui/button.tsx imports from @radix-ui/react-slot
5. Theme tokens work: `className="bg-background text-foreground"` uses CSS variables
6. Dev server runs without errors: `npm run dev`
7. Existing CSS Modules still work (we haven't removed them yet)
</verification>
...
- At least one shadcn component (Button) installed with Radix primitive
