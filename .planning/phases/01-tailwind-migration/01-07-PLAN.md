...
<task type="auto">
  <name>Task 3: Pre-check CSS Module usage and remove unused files</name>
  <files>
  ...
  <action>
**CRITICAL: Pre-check before any deletions**

1. First, verify no component imports CSS Modules (BLOCKING CHECK):
   ```bash
   echo "=== Checking for CSS Module imports in migrated files ==="
   grep -r "\.module\.css" src/components/ src/patterns/ src/App.tsx 2>/dev/null || echo "No CSS Module imports found - safe to proceed"
   ```

   **If any imports are found, STOP and fix them first. Do NOT proceed with deletions.**

2. Document what CSS Module files exist before deletion:
   ```bash
   echo "=== CSS Module files to be deleted ==="
   find src/components src/patterns -name "*.module.css" -type f
   ls src/App.module.css src/index.module.css 2>/dev/null
   ```

3. Only if step 1 returns NO imports, delete migrated CSS Module files in components/:
   ```bash
...
4. Delete migrated pattern CSS Modules:
   ```bash
   rm src/patterns/Header.module.css
   rm src/patterns/Footer.module.css
   ```

5. Delete app-level CSS Modules:
   ```bash
   rm src/App.module.css
   rm src/index.module.css
   ```

6. If variables.css is fully migrated to globals.css, delete it:
   ```bash
   rm src/variables.css  # Only if all variables moved to globals.css
   ```

7. NOTE: Keep page-level CSS Modules (src/pages/*.module.css) for now - they'll be migrated in future phases as pages are enhanced

8. Update .stylelintrc.json if it has CSS Module-specific rules that are no longer needed
...
10. Commit cleanup:
  ...
  <verify>
   - Pre-deletion check passed (no CSS Module imports in migrated components)
   - `npm run build` succeeds after deletions
   - `npm run storybook` works
   - `npm run dev` shows app correctly
   - No TypeScript errors
   - grep -r "\.module\.css" src/components/ src/patterns/ returns empty
  </verify>
  <done>CSS Module files removed after verified safe, Storybook fully updated, migration complete</done>
</task>
